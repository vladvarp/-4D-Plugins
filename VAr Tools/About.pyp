...
------------------------------------------
  ...
  Tools
    ...
    * Clean Objects
      Удаляет все объекты того же типа, что и
      выделенный объект, сохраняя дочерние объекты.

    * Clean Empty Objects
      Удаляет все объекты того же типа, что и
      выделенный объект, НО только если у них нет
      дочерних объектов.
    ...
    * Clean Select Tags
      Удаляет все теги того же типа, что и
      выделенный тег. Поиск ведётся по всей сцене.

    * Clean Empty Mat-Tags
      Удаляет все теги материалов у которых не
      назначен материал. Поиск ведётся по всей
      сцене.

    * Clean Select Mat-Tags
      Удаляет все теги материалов с объектов,
      у которых назначен тот же материал, что
      и у выделенного тега материала.
...
------------------------------------------
  ...
  * Child Selector Teg
    Тег выбора дочернего объекта:
    - Выпадающий список  — прямые дети объекта
    - Имя                — имя выбранного объекта (авто)
    - Связь              — ссылка на выбранный объект (авто)
  ...
  * Camera Visibility Tag
    Тег управляет видимостью объекта (вьюпорт + рендер)
    в зависимости от активной камеры сцены (Stage-объект).
